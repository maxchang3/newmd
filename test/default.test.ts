...
describe('zod Defaults', () => {
    ...
    it('should correctly non-omit optional field values from defaults', () => {
        const schema = z.object({ name: z.string().optional() })
        const schemaDefaults = getDefaultsForSchema(schema)
        expect(typeof schemaDefaults.name).toBeDefined()
        expect(Object.keys(schemaDefaults).indexOf('name')).toBeGreaterThan(-1)
    })
